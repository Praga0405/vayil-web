/**
 * /vendor-studio/profile — v4.5.29
 *
 * The canonical vendor profile editor, rendered inside the VendorStudio
 * chrome (the newer sidebar). Replaces /vendor/profile which has been
 * folded into a server-side redirect (see src/app/vendor/profile/page.tsx).
 *
 * Behaviour:
 *   - Role guard: customers get bounced to /account/profile.
 *   - Profile photo upload uses the shared ProfileImageUploader (JPG /
 *     PNG / WebP, <= 5 MB) and PATCHes the avatar URL onto the row
 *     via vendorApi.saveProfile so it persists across logins.
 *   - Business details (company name, description, email, state, city)
 *     save through vendorApi.saveStep1.
 */
'use client'
...
export default function VendorStudioProfilePage() {
